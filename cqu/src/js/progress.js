...
(function ($) {
        ...
        $("BODY").append(
		  '<div id="processing_container">' +
		    ...
		    '<div id="processing_content">' +
		      '<div id="processing_message">'+ progText+'</div>' +
			  '<div id="processing_pic">'+progImg+'</div>'+
			'</div>' +
		  '</div>');
	...
	$.increasing = function(progress)
	{
		$("#processing_message").text(progress+"%");
	}
	$.autoIncrease = function()
	{
		...
		{
			counter++;
			setTimeout("$.increasing("+counter+")",100*(counter-1));
			
			//$.increasing(counter);
		}
		setTimeout("close_progress()",10000);
	},
